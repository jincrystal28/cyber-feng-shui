...
def scan_nearby_fengshui_pois(lat, lon, radius=500):
    ...
    try:
        ...
        for element in data.get('elements', []):
            ...
            elif natural == 'wood': pois.append(f"{name} (林地/生发之木)")
...
def main():
    ...
    with col1: 
        # accept_multiple_files=True 允许用户框选多张照片
        window_files = st.file_uploader("📸 拍摄窗外环境 (可多张)", type=["jpg", "png", "jpeg"], accept_multiple_files=True, key="win")
    with col2: 
        indoor_files = st.file_uploader("📜 上传室内多角度照片 (可多张)", type=["jpg", "png", "jpeg"], accept_multiple_files=True, key="in")
    ...
    if st.button("🔮 开启禅心风水推演", type="primary", use_container_width=True):
        ...
        with st.status("🌿 沟通天地，正在静心推演...", expanded=True) as status:
            try:
                ...
                # 【重大升级】：重构 Prompt，严禁套用模板，必须动态解析图片
                master_prompt = f"""
                # Role: 首席环境地理学与堪舆宗师 (精通环境心理学与断舍离美学)
                你的诊断必须中正客观，先扬后抑。结合传统理气与现代心理学。

                # Input Data:
                1. 地脉雷达信息：\n{geo_context}
                2. 用户上传的多角度现场照片。

                # Output Format (必须详尽，总字数不少于800字)：
                1. 📜 【禅语定势】：写一首四句七言诗，概括此地气场。
                2. 🌟 【地脉与气场优势 (先扬)】：详细分析雷达数据中的道路、水系、建筑带来的“生机与聚气”吉相。用优美的国学语言让用户感到安心。
                3. ⚠️ 【形峦隐患勘验 (后抑)】：指出雷达或照片中的核心隐患。不仅用风水解释，必须结合“环境心理学”说明其对潜意识的负面影响。
                4. 🧹 【大道至简 · 扫洒除尘】（免费药引 - 极其重要！）：
                   - 严禁套用固定模板（不要每次都说清理柜顶泰山压顶）！
                   - 核心要求：你必须死死盯住用户上传的照片，找出照片中真实存在的某个具体细节（例如：某处堆积的纸箱、乱绕的电线、枯萎的植物、挡光的窗帘、脏污的镜子等）。
